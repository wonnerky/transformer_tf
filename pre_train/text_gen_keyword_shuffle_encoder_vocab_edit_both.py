...
def encode(keyword, text):
    ...
    '''
    input_tokens = tokenizer_encoder.encode(keyword.numpy().decode('utf-8'))
    label_tokens = tokenizer_decoder.encode(text.numpy().decode('utf-8'))
        ...
        for index in index_set:
            ...
            if rng.random() < 0.8:
                ...
                # 10% of the time, replace with random word
                    ...
                    masked_token = rng.randint(1, tokenizer_decoder.vocab_size)
...
def evaluate(inp_sentence):
    start_token = [tokenizer_decoder.vocab_size]
    end_token = [tokenizer_decoder.vocab_size + 1]

    # inp sentence is portuguese, hence adding the start and end token
    inp_sentence = start_token + tokenizer_decoder.encode(inp_sentence) + end_token
    encoder_input = tf.expand_dims(inp_sentence, 0)

    # as the target is english, the first word to the transformer should be the
    # english start token.
    decoder_input = [tokenizer_decoder.vocab_size]
    ...
    for i in range(MAX_LENGTH):
        ...
        if predicted_id == tokenizer_decoder.vocab_size + 1:
...
if __name__ == '__main__':
    ...
    tokenizer_decoder = XLMTokenizer.from_pretrained('xlm-mlm-en-2048')
    tokenizer_encoder = XLMTokenizer.from_pretrained('xlm-mlm-en-2048')
    ...
    input_vocab_size = tokenizer_encoder.vocab_size
    target_vocab_size = tokenizer_decoder.vocab_size
    dropout_rate = 0.1
    ...
    optimizer = tf.keras.optimizers.Adam(0.00009, beta_1=0.9, beta_2=0.999, epsilon=1e-6)
    ...
    directory = "./ckpt/"
    file_name = '1023_stop_wo_pe_shuffle_mask_30_vocab_edit_both'
    checkpoint_path = directory + f'{file_name}/'
    ...
    with open(f'{checkpoint_path}log/{file_name}.json.txt', 'wb') as f:
        pickle.dump(log_, f)