...
'''
def load_wiki_dataset(file_path, flag=1, shuffle=False, isStopword=False):
    train = {}
    test = {}
    valid = {}
    if isStopword:
...
def wiki_dataset(file_path, flag=1, shuffle=False, isStopword=False):
    print(f'dataset mode : flag({flag}), shuffle({shuffle}), isStopword({isStopword})')
    _train, _test, _valid = load_wiki_dataset(file_path, flag, shuffle, isStopword)
...
if __name__ == '__main__':
    ...
    train, test, valid = wiki_dataset(file_path, shuffle=shuffle, isStopword=isStopword)
    ...
    for epoch in range(EPOCHS):
        ...
        for (batch, (inp, tar)) in enumerate(train_dataset):
            ...
            if batch % 300 == 0:
                print('Epoch {} Batch {}/{} Loss {:.12f} Accuracy {:.12f}'.format(epoch + 1, batch, int(total_data_len/BATCH_SIZE), train_loss.result(),
                                                                             train_accuracy.result()))

            if batch % 6000 == 0 and batch != 0:
                ckpt_num = ckpt_num + 1
                ckpt_save_path = ckpt_manager.save()
                print('Saving checkpoint for epoch {} batch {}/{} at {}'.format(epoch + 1, batch, int(total_data_len/BATCH_SIZE), ckpt_save_path))
